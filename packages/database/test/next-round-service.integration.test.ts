...
async function seedConfirmedGroup(): Promise<void> {
  ...
  await client.competition.create({ data: {
    createdById: ids.actor, editionId: ids.edition, eventId: ids.event, formatCode: 'GROUP_STAGE', groupCount: 1,
    id: ids.competition, lockedAt: occurredAt, lockedById: ids.actor, modalityId: ids.modality, revision: 6,
    sportId: ids.sport, status: 'LOCKED', updatedById: ids.actor,
  } });
  ...
  await client.competitionRuleSet.create({ data: {
    canonicalHash: hash, competitionId: ids.competition, createdById: ids.actor, frozenAt: occurredAt, frozenById: ids.actor,
    id: ids.ruleSet, knockoutResolutionCode: 'HIGHER_SCORE', profileConfig: { allowDraws: false, profile: 'SCORE_BASED' },
    resultProfile: 'SCORE_BASED', revisionNumber: 1, schemaVersion: 1, status: 'FROZEN', updatedById: ids.actor,
  } });
  await client.drawConfiguration.create({ data: {
    canonicalHash: hash, competitionId: ids.competition, createdById: ids.actor, formatCode: 'GROUP_STAGE', frozenAt: occurredAt,
    frozenById: ids.actor, groupCount: 1, id: ids.configuration, participantCount: 3, revision: 2, roundNumber: 0,
    ruleSetId: ids.ruleSet, status: 'FROZEN', updatedById: ids.actor,
  } });
  await client.officialDraw.create({ data: {
    algorithmVersion: 'oes-draw-v1', competitionId: ids.competition, configurationId: ids.configuration, confirmedAt: occurredAt,
    confirmedById: ids.confirmer, evidenceHash: hash, evidenceJson: { result: { formatCode: 'GROUP_STAGE', groups: [] } },
    executedAt: occurredAt, executedById: ids.actor, id: ids.execution, resultHash: hash, revision: 2,
    seedCommitment: hash, seedHex: hash, status: 'CONFIRMED',
  } });
  ...
  await client.groupQualification.create({ data: {
    competitionId: ids.competition, confirmedAt: occurredAt, confirmedById: ids.confirmer, firstParticipantId: ids.participantA,
    groupId: ids.group, id: ids.qualification, proposedAt: occurredAt, proposedById: ids.actor, revision: 2,
    secondParticipantId: ids.participantB, sourceRuleSetId: ids.ruleSet, status: 'CONFIRMED',
  } });
}

function prepare(correlationId: string): ReturnType<PrismaNextRoundService['prepare']> {
...
integration('PrismaNextRoundService', () => {
  beforeEach(async () => { await clean(); await seedConfirmedGroup(); });
  afterAll(async () => { await clean(); await client.$disconnect(); });

  it('freezes round one only from the confirmed group qualifiers', async () => {
    const prepared = await prepare(ids.correlation);
    expect(prepared.competitionRevision).toBe(7);
    expect(prepared.configuration).toMatchObject({
      competitionId: ids.competition, formatCode: 'KNOCKOUT', groupCount: null, participantCount: 2,
      roundNumber: 1, ruleSetId: ids.ruleSet, status: 'FROZEN',
    });
    ...
    expect(await client.auditEntry.findFirst({ where: { actionCode: 'NEXT_ROUND_CONFIGURATION_FROZEN', competitionId: ids.competition } })).toMatchObject({ actorId: ids.actor, resourceId: prepared.configuration.id });
  ...
  it('does not create the round while qualification is pending', async () => {
    await client.groupQualification.update({ data: { confirmedAt: null, confirmedById: null, revision: 1, status: 'PENDING_CONFIRMATION' }, where: { id: ids.qualification } });
    await expect(prepare(ids.correlation)).rejects.toMatchObject({ code: 'DRAW_CONFIGURATION_INCOMPATIBLE' });
  ...
  it('allows exactly one concurrent preparation for the same competition revision', async () => {
    const race = await Promise.allSettled([
      prepare('e1000000-0000-4000-8000-000000000011'),
      prepare('e1000000-0000-4000-8000-000000000012'),
    ]);
    ...
    expect(rejected[0]).toMatchObject({ reason: { code: 'CONCURRENCY_CONFLICT' } });
    expect(await client.drawConfiguration.count({ where: { competitionId: ids.competition, formatCode: 'KNOCKOUT', roundNumber: 1, status: 'FROZEN' } })).toBe(1);
    expect(await client.auditEntry.count({ where: { actionCode: 'NEXT_ROUND_CONFIGURATION_FROZEN', competitionId: ids.competition } })).toBe(1);
    expect(await client.competition.findUnique({ where: { id: ids.competition } })).toMatchObject({ revision: 7, status: 'LOCKED' });
  });
});
