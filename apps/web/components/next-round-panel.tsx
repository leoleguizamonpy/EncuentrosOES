...
export function NextRoundPanel({
  ...
  async function prepare(): Promise<void> {
    ...
    setSubmitting(true);
    try {
      await prepareNextRound(competitionId, draw.competitionRevision);
      onChange(await drawWorkspace(competitionId));
    } catch (caught: unknown) {
      onError(caught instanceof Error ? caught.message : 'No fue posible preparar la siguiente ronda.');
    } finally {
      setSubmitting(false);
    }
  }

  const nextRound = draw.configuration?.formatCode === 'KNOCKOUT'
    ? draw.configuration.roundNumber + 1
    : 1;

  return <section className="setup-card" aria-labelledby="next-round-title">
    <div className="section-title">
      <div><span className="eyebrow eyebrow--dark">Continuidad</span><h3 id="next-round-title">Siguiente ronda eliminatoria</h3></div>
      <span>{String(nextRound).padStart(2, '0')}</span>
    </div>
    ...
    {canOperate
      ? <button className="primary-button" disabled={submitting} onClick={() => void prepare()} type="button">{submitting ? 'Preparando…' : `Preparar ronda ${String(nextRound)}`}</button>
      : <p className="readonly-note">Un administrador debe preparar la siguiente ronda.</p>}
  </section>;
}
