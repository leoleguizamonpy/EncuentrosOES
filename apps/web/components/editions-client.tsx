...
function EditionsWorkspace(): React.JSX.Element {
  ...
  return <div className={styles.workspace}>
    <section className={styles.heading}><div><span className="eyebrow eyebrow--dark">Organización</span><h2>Ediciones</h2><p>Administra cada ciclo anual de OES y controla cuándo una edición queda abierta o cerrada para su operación.</p></div><Button onPress={startCreate} variant="primary">+ Nueva edición</Button></section>
    ...
    <Card className={styles.tableCard ?? ''} aria-label="Listado de ediciones"><Card.Content style={{ padding: 0 }}><div className={styles.tableHeader}><span>Año</span><span>Edición</span><span>Ciclo</span><span>Estado</span><span>Acción</span></div>{filtered.length === 0 ? <div className={styles.empty}><strong>{catalog.editions.length === 0 ? 'No hay ediciones todavía.' : 'No encontramos resultados.'}</strong><p>{catalog.editions.length === 0 ? 'Crea la edición que agrupará los eventos y competencias de un ciclo OES.' : 'Ajusta la búsqueda o el filtro para ver otras ediciones.'}</p>{catalog.editions.length === 0 ? <Button onPress={startCreate} variant="primary">+ Nueva edición</Button> : null}</div> : filtered.map((item) => <article className={styles.row} key={item.id}><span className={styles.logo}>{String(item.year).slice(-2)}</span><div className={styles.identity}><strong>{item.name}</strong><small>Edición OES</small></div><span className={styles.eventName}>{item.year}</span><Chip color={item.status === 'OPEN' ? 'success' : 'default'} size="sm" variant="soft">{item.status === 'OPEN' ? 'Abierta' : 'Cerrada'}</Chip><Button onPress={() => startEdit(item)} size="sm" variant="secondary">Editar</Button></article>)}</Card.Content></Card>
