...
function VisualDrawer({ item, kind, onClose, onSaved }: DrawerProps): React.JSX.Element {
  ...
  return <>
    ...
    <aside aria-labelledby="visual-drawer-title" aria-modal="true" className={styles.drawer} role="dialog">
      <div className={styles.drawerHeader}><div><span className="eyebrow eyebrow--dark">Organización</span><h3 id="visual-drawer-title">{item === null ? labels.createLabel.replace('+ ', '') : `Editar ${labels.singular}`}</h3></div><button aria-label="Cerrar" className={styles.closeButton} onClick={onClose} type="button">×</button></div>
      {error === null ? null : <p className={styles.error} role="alert">{error}</p>}
      <form className={styles.form} onSubmit={(event) => void submit(event)}>
        <label>Nombre *<input required value={name} onChange={(event) => setName(event.target.value)} placeholder={labels.placeholderName} /></label>
        <label>Código *<input required value={code} onChange={(event) => setCode(event.target.value)} placeholder={labels.placeholderCode} /></label>
        ...
        <div className={styles.actions}><button className={styles.secondaryButton} onClick={onClose} type="button">Cancelar</button><button className={styles.saveButton} disabled={saving} type="submit">{saving ? 'Guardando…' : `Guardar ${labels.singular}`}</button></div>
      </form>
    </aside>
  </>;
}
...
function VisualCatalogWorkspace({ kind }: { readonly kind: VisualCatalogKind }): React.JSX.Element {
  ...
  if (loading) return <div className="empty-state"><strong>{labels.loadingLabel}</strong><p>Recuperando la organización desde el servidor.</p></div>;
  ...
  return <div className={styles.workspace}>
    <section className={styles.heading}><div><span className="eyebrow eyebrow--dark">Organización</span><h2>{labels.plural}</h2><p>{labels.description}</p></div><button className={styles.primaryButton} onClick={startCreate} type="button">{labels.createLabel}</button></section>
    ...
    <section aria-label={`Filtros de ${labels.plural.toLocaleLowerCase('es-PY')}`} className={styles.toolbar}><input aria-label={`Buscar ${labels.singular}`} placeholder="Buscar por nombre o código…" value={query} onChange={(event) => setQuery(event.target.value)} /><select aria-label="Filtrar por estado" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as StatusFilter)}><option value="ALL">Todos los estados</option><option value="ACTIVE">Activos</option><option value="INACTIVE">Inactivos</option></select><span /><span className={styles.counter}>{filtered.length} de {items.length}</span></section>
    <section aria-label={`Listado de ${labels.plural.toLocaleLowerCase('es-PY')}`} className={styles.tableCard}><div className={styles.tableHeader}><span>Icono</span><span>{labels.singular}</span><span>Código</span><span>Estado</span><span>Acción</span></div>{filtered.length === 0 ? <div className={styles.empty}><strong>{items.length === 0 ? labels.emptyLabel : 'No encontramos resultados.'}</strong><p>{items.length === 0 ? labels.emptyDescription : 'Ajusta la búsqueda o el filtro para ver otros registros.'}</p>{items.length === 0 ? <button className={styles.primaryButton} onClick={startCreate} type="button">{labels.createLabel}</button> : null}</div> : filtered.map((item) => <article className={styles.row} key={item.id}><span className={styles.logo}>{item.iconAssetId === null ? item.code.slice(0, 2) : <img alt={`Icono de ${item.name}`} src={catalogAssetUrl(item.iconAssetId)} />}</span><div className={styles.identity}><strong>{item.name}</strong><small>{labels.itemLabel}</small></div><span className={styles.eventName}>{item.code}</span><span className={[styles.status, item.active ? styles.active : styles.inactive].filter(Boolean).join(' ')}>{item.active ? 'Activo' : 'Inactivo'}</span><button className={styles.editButton} onClick={() => startEdit(item)} type="button">Editar</button></article>)}</section>
