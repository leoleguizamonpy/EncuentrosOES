...
function MatchCard({ actorId, canAnnul, canOperate, canSelfConfirm, match, onChange, onError, profile }: {
  ...
  return <Card className="result-match">
    ...
    <Card.Content>
      ...
      {match.result?.status === 'CONFIRMED' && canAnnul ? <div className="result-annulment">{!annulmentOpen ? <Button isDisabled={submitting !== null} onPress={() => setAnnulmentOpen(true)} variant="secondary">Anular resultado</Button> : <Alert status="danger"><Alert.Indicator /><Alert.Content><Alert.Title>Anulación oficial del resultado</Alert.Title><label htmlFor={`result-annulment-${match.result.id}`}>Motivo formal de anulación</label><textarea id={`result-annulment-${match.result.id}`} maxLength={500} minLength={10} onChange={(event) => setAnnulmentReason(event.target.value)} placeholder="Explica el error que obliga a cargar nuevamente este resultado…" value={annulmentReason} /><small>{annulmentReason.trim().length}/500 · mínimo 10 caracteres</small><div style={{ display: 'flex', gap: 8, marginTop: 10 }}><Button isDisabled={submitting !== null || annulmentReason.trim().length < 10} onPress={() => void annul()} size="sm" variant="primary">{submitting === 'annul' ? 'Anulando…' : 'Confirmar anulación'}</Button><Button isDisabled={submitting !== null} onPress={() => { setAnnulmentOpen(false); setAnnulmentReason(''); }} size="sm" variant="ghost">Cancelar</Button></div></Alert.Content></Alert>}</div> : null}
    </Card.Content>
  </Card>;
}
