...
function standingColumns(setBased: boolean): readonly DataTableColumn<StandingRowView>[] {
  ...
  if (!setBased) shared.push({ id: 'draws', label: 'E', render: (row) => row.draws });
  shared.push({ id: 'losses', label: 'P', render: (row) => row.losses }, { id: 'points', label: 'Pts.', render: (row) => <strong>{row.tablePoints}</strong> });
  if (setBased) shared.push({ id: 'sets', label: 'SG', render: (row) => row.setsWon }, { id: 'difference', label: 'DP', render: (row) => row.sportPointDifference });
  else shared.push({ id: 'for', label: 'GF', render: (row) => row.scoreFor }, { id: 'against', label: 'GC', render: (row) => row.scoreAgainst }, { id: 'difference', label: 'DG', render: (row) => row.scoreDifference });
  return shared;
}
...
export function ResultsWorkspacePanel({ actorId, canAnnul, canOperate, canSelfConfirm = false, onChange, onError, workspace }: { readonly actorId: string; readonly canAnnul: boolean; readonly canOperate: boolean; readonly canSelfConfirm?: boolean; readonly onChange: (workspace: ResultsWorkspace) => void; readonly onError: (message: string | null) => void; readonly workspace: ResultsWorkspace }): React.JSX.Element {
  ...
  return <SectionPanel className="results-workspace" id="results-workspace" eyebrow="Paso 5" title="Encuentros y tabla" status={<Chip size="sm" variant="soft">{workspace.matches.length} encuentros</Chip>}>
    {workspace.matches.length === 0 ? <div className="setup-empty">Los encuentros aparecerán cuando se confirme el sorteo oficial.</div> : <>
      <div className="result-match-list">{workspace.matches.map((match) => <MatchCard actorId={actorId} canAnnul={canAnnul} canOperate={canOperate} canSelfConfirm={canSelfConfirm} key={match.id} match={match} onChange={onChange} onError={onError} profile={workspace.resultProfile} />)}</div>
      {workspace.groups.map((group) => <Card className="standing-card" key={group.id}>
        <Card.Header><div><span>Tabla automática</span><strong>Grupo {group.label}</strong></div><Chip color={group.complete ? 'success' : 'default'} size="sm" variant="soft">{group.complete ? 'Completa' : 'Parcial'}</Chip></Card.Header>
        <Card.Content>
          <DataTable columns={standingColumns(setBased)} getRowKey={(row) => row.participant.id} label={`Tabla del grupo ${group.label}`} rows={group.standings} width={setBased ? 'medium' : 'wide'} />
