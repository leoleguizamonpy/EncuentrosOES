    ...
    <Card.Content>
      ...
      {configuration === null ? <div className="draw-empty">
        ...
        {execution.status === 'CONFIRMED' ? <>
          ...
          {canAnnul ? <div className="draw-annulment">{!annulmentOpen ? <Button isDisabled={action !== null} onPress={() => setAnnulmentOpen(true)} variant="secondary">Anular sorteo oficial</Button> : <Alert status="danger"><Alert.Indicator /><Alert.Content><Alert.Title>Esta acción revoca la publicación y conserva el historial.</Alert.Title><label htmlFor={`annulment-reason-${execution.id}`}>Motivo formal de anulación</label><textarea id={`annulment-reason-${execution.id}`} maxLength={500} minLength={10} onChange={(event) => setAnnulmentReason(event.target.value)} placeholder="Explica el error que obliga a reemplazar este sorteo…" value={annulmentReason} /><small>{annulmentReason.trim().length}/500 · mínimo 10 caracteres</small><div style={{ display: 'flex', gap: 8, marginTop: 10 }}><Button isDisabled={action !== null || annulmentReason.trim().length < 10} onPress={() => void annul()} size="sm" variant="primary">{action === 'annul' ? 'Anulando…' : 'Confirmar anulación'}</Button><Button isDisabled={action !== null} onPress={() => { setAnnulmentOpen(false); setAnnulmentReason(''); }} size="sm" variant="ghost">Cancelar</Button></div></Alert.Content></Alert>}</div> : null}
