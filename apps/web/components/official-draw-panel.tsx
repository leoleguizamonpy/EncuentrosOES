...
export function OfficialDrawPanel({
  ...
  return <Card className="setup-card official-draw-card" id="official-draw-workspace" aria-labelledby="official-draw-title">
    <Card.Content>
      <div className="section-title"><div><span className="eyebrow eyebrow--dark">Paso 4</span><h3 id="official-draw-title">Sorteo oficial</h3></div><Chip color={drawColor} size="sm" variant="soft">{drawState}</Chip></div>
      {configuration === null ? <div className="draw-empty">
        ...
        {!readyToPrepare ? <p className="readonly-note">Guarda el formato y congela la plantilla de puntuación antes de preparar el sorteo.</p> : !canPrepare ? <p className="readonly-note">Tu rol o el estado actual solo permiten consultar el sorteo.</p> : !confirmLock ? <Button onPress={() => setConfirmLock(true)} variant="primary">Preparar sorteo oficial</Button> : <Alert status="warning"><Alert.Indicator /><Alert.Content><Alert.Title>Bloqueo irreversible para esta ejecución</Alert.Title><Alert.Description>Verifica participantes, formato y reglas antes de continuar.</Alert.Description><div style={{ display: 'flex', gap: 8, marginTop: 10 }}><Button isDisabled={action !== null} onPress={() => void run('prepare')} size="sm" variant="primary">{action === 'prepare' ? 'Preparando…' : 'Confirmar y bloquear'}</Button><Button isDisabled={action !== null} onPress={() => setConfirmLock(false)} size="sm" variant="ghost">Cancelar</Button></div></Alert.Content></Alert>}
        ...
        {execution.status === 'CONFIRMED' ? <>
          ...
          {canAnnul ? <div className="draw-annulment">{!annulmentOpen ? <Button color="danger" isDisabled={action !== null} onPress={() => setAnnulmentOpen(true)} variant="secondary">Anular sorteo oficial</Button> : <Alert status="danger"><Alert.Indicator /><Alert.Content><Alert.Title>Esta acción revoca la publicación y conserva el historial.</Alert.Title><label htmlFor={`annulment-reason-${execution.id}`}>Motivo formal de anulación</label><textarea id={`annulment-reason-${execution.id}`} maxLength={500} minLength={10} onChange={(event) => setAnnulmentReason(event.target.value)} placeholder="Explica el error que obliga a reemplazar este sorteo…" value={annulmentReason} /><small>{annulmentReason.trim().length}/500 · mínimo 10 caracteres</small><div style={{ display: 'flex', gap: 8, marginTop: 10 }}><Button color="danger" isDisabled={action !== null || annulmentReason.trim().length < 10} onPress={() => void annul()} size="sm" variant="primary">{action === 'annul' ? 'Anulando…' : 'Confirmar anulación'}</Button><Button isDisabled={action !== null} onPress={() => { setAnnulmentOpen(false); setAnnulmentReason(''); }} size="sm" variant="ghost">Cancelar</Button></div></Alert.Content></Alert>}</div> : null}
