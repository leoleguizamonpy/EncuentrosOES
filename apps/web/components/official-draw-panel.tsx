...
export function OfficialDrawPanel({
  ...
  function openPrintableAct(publicationId: string): void {
    window.open(`/draws/${publicationId}?print=1`, '_blank', 'noopener,noreferrer');
  }
  ...
  return <SectionPanel className="official-draw-card" id="official-draw-workspace" eyebrow="Paso 4" title="Sorteo oficial" status={<Chip color={drawColor} size="sm" variant="soft">{drawState}</Chip>}>
    {configuration === null ? <div className="draw-empty">
      ...
      {execution.status === 'CONFIRMED' ? <>
        ...
        {workspace.publication === null ? canOperate ? <Button isDisabled={action !== null} onPress={() => void run('publish')} variant="primary">{action === 'publish' ? 'Publicando…' : 'Publicar sorteo y acta'}</Button> : <p className="readonly-note">Esperando que una autoridad publique el sorteo.</p> : <Card variant="tertiary"><Card.Content className="draw-publication-proof"><strong>Publicado y verificable</strong><code>{workspace.publication.verificationCode}</code><InlineActions compact><a href={`/draws/${workspace.publication.id}`}>Abrir vista pública</a><Button onPress={() => openPrintableAct(workspace.publication?.id ?? '')} size="sm" variant="secondary">Imprimir acta</Button><Button onPress={() => openPrintableAct(workspace.publication?.id ?? '')} size="sm" variant="primary">Descargar PDF</Button><a href={publicDrawActUrl(workspace.publication.id)}>Descargar acta JSON</a></InlineActions><small>“Descargar PDF” abre la salida A4 certificada para guardarla como PDF desde el navegador.</small></Card.Content></Card>}
