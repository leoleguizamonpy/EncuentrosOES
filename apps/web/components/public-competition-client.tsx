...
export function PublicCompetitionClient({ competitionId }: { readonly competitionId: string }): React.JSX.Element {
  ...
  return <main id="main-content" className="public-draw-shell">
    <header className="public-draw-header"><OesMark /><div><span>{finalized ? 'Competencia oficial finalizada' : 'Competencia oficial en curso'}</span><h1>{journey.competition.sport} · {journey.competition.modality}</h1><p>{journey.competition.edition} / {journey.competition.event}</p></div><Chip color={finalized ? 'success' : 'accent'} size="sm" variant="soft">{finalized ? '✓ Finalizada' : '● En curso'}</Chip></header>
