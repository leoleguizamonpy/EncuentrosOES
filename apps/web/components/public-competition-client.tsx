...
export function PublicCompetitionClient({ competitionId }: { readonly competitionId: string }): React.JSX.Element {
  ...
  if (error !== null) return <main id="main-content" className="public-draw-shell"><OesMark /><section className="public-draw-error" role="alert"><h1>Competencia no disponible</h1><p>{error}</p></section></main>;
  if (journey === undefined) return <main id="main-content" className="public-draw-shell"><OesMark /><p className="public-draw-loading" role="status" aria-live="polite">Restaurando competencia oficial…</p></main>;
  if (journey === null) return <main id="main-content" className="public-draw-shell"><OesMark /><section className="public-draw-error" role="status"><h1>Aún sin publicación oficial</h1><p>La competencia aparecerá aquí cuando al menos una ronda haya sido publicada oficialmente.</p></section></main>;
  ...
  return <main id="main-content" className="public-draw-shell">
    <header className="public-draw-header">
      ...
      <strong className="public-state-label" role="status" aria-label={finalized ? 'Estado: finalizada' : 'Estado: en curso'}>{finalized ? '✓ Finalizada' : '● En curso'}</strong>
    </header>
    ...
    {journey.rounds.map((round) => {
      ...
      return <section key={round.executionId} className="public-draw-verification" aria-labelledby={`round-${round.executionId}`}>
        <h2 className="public-round-heading" id={`round-${round.executionId}`}>{label}</h2>
