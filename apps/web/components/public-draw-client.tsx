...
export function PublicDrawClient({ publicationId }: { readonly publicationId: string }): React.JSX.Element {
  ...
  return <main id="main-content" className="public-draw-shell">
    <header className="public-draw-header"><OesMark /><div><span>Acta oficial de sorteo</span><h1>{act.competition.sport} · {act.competition.modality}</h1><p>{act.competition.edition} / {act.competition.event}</p></div><div className="public-draw-actions"><Chip color={publication.verified ? 'success' : 'danger'} size="sm" variant="soft">{publication.verified ? '✓ Verificada' : 'No verificable'}</Chip><PrintAction label="Imprimir acta" /><PrintAction label="Descargar PDF" /></div></header>
    ...
    <Card className="public-draw-verification" aria-labelledby="verification-heading"><Card.Content><h2 className="public-round-heading" id="verification-heading">Verificación oficial</h2><span>Código SHA-256</span><code>{publication.verificationCode}</code><p>Este código protege la competencia, participantes, reglas congeladas, resultado, semilla y datos del acta.</p><a href={publicDrawActUrl(publication.id)}>Descargar acta verificable</a><Link href={`/draws/${publication.id}/presentation?step=0`}>Abrir modo presentación</Link><small>Para PDF, usa “Descargar PDF” y selecciona Guardar como PDF en el diálogo del navegador.</small></Card.Content></Card>
