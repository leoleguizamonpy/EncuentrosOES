...
export function PublicDrawClient({ publicationId }: { readonly publicationId: string }): React.JSX.Element {
  ...
  return <main className="public-draw-shell">
    ...
    <section className="public-draw-verification"><span>Código de verificación SHA-256</span><code>{publication.verificationCode}</code><p>Este código protege la competencia, participantes, reglas congeladas, resultado, semilla y datos del acta.</p><a href={publicDrawActUrl(publication.id)}>Descargar acta verificable</a></section>
  </main>;
}
