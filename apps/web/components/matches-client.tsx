...
function MatchesWorkspace(): React.JSX.Element {
  ...
  return <PageLayout>
    <PageHeader description="Consulta marcador, fase, contexto y estado operativo de cada partido desde una sola vista." eyebrow="Competencia" title="Encuentros" trailing={<span aria-label={`${String(pendingResults)} sin resultado y ${String(pendingConfirmations)} por confirmar`}><StatusBadge label={`${String(pendingResults)} sin resultado`} tone={pendingResults > 0 ? 'warning' : 'success'} /> <StatusBadge label={`${String(pendingConfirmations)} por confirmar`} tone={pendingConfirmations > 0 ? 'warning' : 'success'} /></span>} />
    {failedCompetitionCount === 0 ? null : <Notice description={`No fue posible recuperar los encuentros de ${String(failedCompetitionCount)} ${failedCompetitionCount === 1 ? 'competencia' : 'competencias'}. Los datos disponibles de las demás competencias siguen visibles.`} title="Datos parciales" tone="warning" />}
