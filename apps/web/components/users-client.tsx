...
function UserDrawer({ item, onClose, onSaved }: { readonly item: ManagedUser | null; readonly onClose: () => void; readonly onSaved: (user: ManagedUser) => void }): React.JSX.Element {
  ...
  return <div className={styles.drawerBackdrop} role="presentation"><aside aria-label={item === null ? 'Nuevo usuario' : 'Editar usuario'} className={styles.drawer}>
    <header className={styles.drawerHeader}><div><span className="eyebrow eyebrow--dark">Control</span><h3>{item === null ? 'Nuevo usuario' : 'Editar usuario'}</h3></div><Button aria-label="Cerrar" isIconOnly onPress={onClose} variant="ghost">×</Button></header>
    <form className={styles.form} onSubmit={(event) => void save(event)}>
      <label>Nombre visible *<Input required maxLength={120} minLength={2} value={draft.displayName} onChange={(event) => setDraft((current) => ({ ...current, displayName: event.target.value }))} variant="secondary" /></label>
      <label>Correo electrónico *<Input required isDisabled={item !== null} maxLength={254} type="email" value={draft.email} onChange={(event) => setDraft((current) => ({ ...current, email: event.target.value }))} variant="secondary" /></label>
      <label>{item === null ? 'Contraseña inicial *' : 'Nueva contraseña (opcional)'}<Input required={item === null} minLength={12} maxLength={256} type="password" value={draft.password} onChange={(event) => setDraft((current) => ({ ...current, password: event.target.value }))} variant="secondary" /></label>
      ...
      <footer className={styles.drawerActions}><Button isDisabled={saving} onPress={onClose} type="button" variant="secondary">Cancelar</Button><Button isDisabled={saving} type="submit" variant="primary">{saving ? 'Guardando…' : 'Guardar usuario'}</Button></footer>
    </form>
  </aside></div>;
}
...
function UsersWorkspace(): React.JSX.Element {
  ...
  return <div className={styles.workspace}>
    <section className={styles.heading}><div><span className="eyebrow eyebrow--dark">Control</span><h2>Usuarios</h2><p>Administra cuentas, roles y acceso. Solo el SUPERADMIN puede modificar esta superficie; cambios de rol, estado o contraseña invalidan sesiones vigentes.</p></div><Button onPress={() => setDrawer('NEW')} variant="primary">+ Nuevo usuario</Button></section>
    {error === null ? null : <Alert status="danger" role="alert"><Alert.Indicator /><Alert.Content><Alert.Description>{error}</Alert.Description></Alert.Content></Alert>}
    <section aria-label="Filtros de usuarios" className={styles.toolbar}><Input aria-label="Buscar usuario" placeholder="Buscar nombre, correo o rol…" value={query} onChange={(event) => setQuery(event.target.value)} variant="secondary" /><select aria-label="Filtrar usuarios por estado" value={filter} onChange={(event) => setFilter(event.target.value as UserFilter)}><option value="ALL">Todos</option><option value="ACTIVE">Activos</option><option value="DISABLED">Desactivados</option></select><span/><span className={styles.counter}>{filtered.length} de {items.length}</span></section>
    <Card className={styles.tableCard ?? ''} aria-label="Listado de usuarios"><Card.Content style={{ padding: 0 }}><div className={styles.tableHeader}><span>Cuenta</span><span>Usuario</span><span>Rol</span><span>Estado</span><span>Acción</span></div>{filtered.length === 0 ? <div className={styles.empty}><strong>{items.length === 0 ? 'Aún no hay usuarios.' : 'No encontramos usuarios.'}</strong><p>{items.length === 0 ? 'Crea la primera cuenta administrada.' : 'Ajusta la búsqueda o el filtro.'}</p></div> : filtered.map((item) => <article className={styles.row} key={item.id}><span className={styles.logo}>{item.displayName.charAt(0).toUpperCase()}</span><div className={styles.identity}><strong>{item.displayName}</strong><small>{item.email}{item.lastLoginAt === null ? ' · nunca ingresó' : ` · último acceso ${new Intl.DateTimeFormat('es-PY', { dateStyle: 'short' }).format(new Date(item.lastLoginAt))}`}</small></div><span className={styles.eventName}>{roleLabels[item.role]}</span><Chip color={item.status === 'ACTIVE' ? 'success' : 'default'} size="sm" variant="soft">{item.status === 'ACTIVE' ? 'Activo' : 'Desactivado'}</Chip><Button onPress={() => setDrawer(item)} size="sm" variant="secondary">Editar</Button></article>)}</Card.Content></Card>
